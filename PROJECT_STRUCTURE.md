...
Company branding stores the authorized-signature image on the tenant-private local disk with validated PNG/JPEG/WEBP input and a 1 MB limit. This is a visual approval image, not a cryptographic digital signature. Signature paths, signatory name, designation, and display choice are snapshotted per document, so later branding changes cannot alter historical output. Old signature files are retained while any invoice, quotation, or proforma snapshot references them. The expanded `InvoiceTemplateRegistry` contains 44 A4, A5, and purpose-built thermal templates with paper, style, GST-presentation, tax-mode, and signature compatibility metadata. The design picker filters by paper, tax compatibility, GST detail, and style without rendering full documents for every card.

`invoice_template_settings` also stores tenant-scoped reusable payment instructions and the current private watermark path. Account numbers and per-document payment snapshots are encrypted. `SalesDocumentPresentationService` captures enabled payment details, watermark path, and capture time on each new invoice, quotation, or proforma; `InvoiceWatermarkService` validates and stores generated PNG/JPEG/WEBP assets under the tenant directory and retains replaced files while snapshots reference them. A4, A5, and Thermal 80mm use a centered 12% watermark in browser/PDF output. Thermal 58mm intentionally omits the watermark and renders only a compact payment identifier to preserve receipt readability.
...
Phase R extends the existing inventory domain rather than creating a parallel stock system. `InventoryLocationAccessService` is the shared company/outlet/warehouse authorization boundary. `StockTransferService`, `StockCountService`, `InventoryTraceabilityService`, `InventoryStockViewService`, and `BarcodeRenderer` own transfer accounting, physical/cycle counts, batch/serial identity, authorized operational views/reports, and scanner-readable browser labels.

The additive `2026_08_07_010000_add_advanced_inventory_warehouse_foundation.php` migration extends products, stock levels, movements, transfers, adjustments, reorder rules, POS registers, and barcode print items. It creates transfer receipt/discrepancy, stock count, batch, serial, and transfer-serial tables. Existing registers are assigned an explicit branch warehouse without altering historical POS records.

Inventory routes under `/inventory` now include global availability, product stock detail, the complete Move Stock lifecycle, counts, traceability, and shared inventory reports. All mutations are tenant/location authorized, transactional, audited, and append ledger movements. Dispatch removes source availability into an explicit in-transit state; receipt alone adds destination usable/damaged stock. See `docs/phase-r-advanced-inventory-warehouse.md` for the readiness audit, schema, lifecycle, permissions, concurrency controls, reports, device behavior, deployment notes, and V1 limitations.

# Phase S - Smart Purchase Automation

Phase S extends the existing suppliers, purchase requests, purchase orders, GRNs, supplier invoices, approval logs, and Phase R stock ledger. It does not introduce a parallel purchasing or inventory domain. `PurchaseAccessService` is the shared tenant/outlet/warehouse scope applied to purchasing reads, while the existing `InventoryLocationAccessService` authorizes each warehouse mutation.

The additive `2026_08_14_010000_add_smart_purchase_automation_controls.php` migration adds request submission/conversion evidence, supplier confirmation, GRN idempotency/posting metadata, accepted/rejected/damaged receipt quantities, batch references, normalized invoice-match exceptions, and related MySQL-safe indexes. The migration deliberately retains its audit columns on rollback so production history is never erased and reruns remain additive.
