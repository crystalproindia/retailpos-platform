...
class AttendanceCalendarController extends Controller
{
    ...
    public function storeHoliday(Request $request): RedirectResponse { $data = $request->validate(['name' => ['required', 'string', 'max:120'], 'holiday_date' => ['required', 'date'], 'outlet_id' => ['nullable', 'integer'], 'holiday_type' => ['required', 'in:paid,unpaid,observance'], 'notes' => ['nullable', 'string', 'max:1000']]); $this->assertOutlet($request, $data['outlet_id'] ?? null); $duplicate = Holiday::query()->where('company_id', $request->user()->company_id)->whereDate('holiday_date', $data['holiday_date'])->when($data['outlet_id'] ?? null, fn ($query, $outlet) => $query->where('outlet_id', $outlet), fn ($query) => $query->whereNull('outlet_id'))->exists(); if ($duplicate) return back()->withErrors(['holiday_date' => 'A holiday already exists for this scope and date.']); $holiday = Holiday::create($data + ['company_id' => $request->user()->company_id, 'is_active' => true, 'created_by' => $request->user()->id]); $this->audit->record('attendance.holiday.created', $holiday, 'Holiday calendar updated'); return back()->with('success', 'Holiday added.'); }
    public function storeWeeklyOff(Request $request): RedirectResponse { $data = $request->validate(['weekday' => ['required', 'integer', 'between:1,7'], 'outlet_id' => ['nullable', 'integer'], 'notes' => ['nullable', 'string', 'max:500']]); $this->assertOutlet($request, $data['outlet_id'] ?? null); $duplicate = WeeklyOff::query()->where('company_id', $request->user()->company_id)->where('weekday', $data['weekday'])->when($data['outlet_id'] ?? null, fn ($query, $outlet) => $query->where('outlet_id', $outlet), fn ($query) => $query->whereNull('outlet_id'))->whereNull('employee_id')->exists(); if ($duplicate) return back()->withErrors(['weekday' => 'A company or outlet weekly-off rule already exists for that day.']); $weeklyOff = WeeklyOff::create($data + ['company_id' => $request->user()->company_id, 'is_active' => true, 'created_by' => $request->user()->id]); $this->audit->record('attendance.weekly_off.created', $weeklyOff, 'Weekly off rule added'); return back()->with('success', 'Weekly off rule added'); }
