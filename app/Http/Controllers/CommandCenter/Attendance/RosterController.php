...
class RosterController extends Controller
{
    ...
    public function index(Request $request): View { $access = app(\App\Services\Outlets\OutletAccessService::class); $outlets = $access->accessibleOutlets($request->user()); $requestedOutlet = $request->integer('outlet_id'); if ($requestedOutlet && ! $outlets->contains('id', $requestedOutlet)) abort(404); $outlet = $requestedOutlet ? $outlets->firstWhere('id', $requestedOutlet) : $outlets->first(); $week = CarbonImmutable::parse($request->input('week', now()), $outlet?->timezone ?: config('app.timezone'))->startOfWeek(); $employees = WorkforceEmployee::query()->where('company_id', $request->user()->company_id)->when($outlet, fn ($q) => $q->where('primary_branch_id', $outlet->id))->where('status', 'active')->with(['shiftAssignments' => fn ($q) => $q->with('shift')->whereBetween('work_date', [$week->toDateString(), $week->addDays(6)->toDateString()])])->orderBy('display_name')->get(); $shifts = ShiftTemplate::query()->where('company_id', $request->user()->company_id)->where('is_active', true)->when($outlet, fn ($query) => $query->where(fn ($scoped) => $scoped->whereNull('applicable_outlet_id')->orWhere('applicable_outlet_id', $outlet->id)))->get(); return view('command-center.attendance.roster', compact('outlets', 'outlet', 'week', 'employees', 'shifts')); }
    public function storeShift(Request $request): RedirectResponse { $data = $request->validate(['name' => ['required', 'string', 'max:100'], 'code' => ['required', 'string', 'max:40'], 'start_time' => ['required', 'date_format:H:i'], 'end_time' => ['required', 'date_format:H:i', 'different:start_time'], 'standard_work_minutes' => ['required', 'integer', 'min:1', 'max:1440'], 'overtime_after_minutes' => ['required', 'integer', 'min:1', 'max:1440'], 'outlet_id' => ['nullable', 'integer']]); $access = app(\App\Services\Outlets\OutletAccessService::class); $outletId = $data['outlet_id'] ?? null; $outlet = $outletId ? $access->accessibleOutlets($request->user())->firstWhere('id', $outletId) : $access->current($request->user()); if (! $outlet) abort(404); $this->rosters->createTemplate($request->user(), $data + ['outlet_id' => $outlet->id, 'crosses_midnight' => $request->boolean('crosses_midnight'), 'unpaid_break_minutes' => (int) $request->input('unpaid_break_minutes', 0), 'grace_before_minutes' => (int) $request->input('grace_before_minutes', 0), 'grace_after_minutes' => (int) $request->input('grace_after_minutes', 0), 'minimum_work_minutes' => (int) $request->input('minimum_work_minutes', 0)]); return back()->with('success', 'Shift template created.'); }
