...
{
    public function index(Request $request, CmsCaseStudyRepository $studies): View { return view('command-center.cms.case-studies.index', ['studies' => $studies->paginateForCompany($request->user()->company_id, $request->only(['search', 'status', 'trashed']))]); }
    public function create(Request $request): View { return view('command-center.cms.case-studies.create', ['study' => null, 'media' => CmsMedia::query()->where('company_id', $request->user()->company_id)->orderBy('file_name')->get()]); }
    public function store(CmsCaseStudyRequest $request, CmsCaseStudyService $service): RedirectResponse { $study = $service->create($request->user(), $this->payload($request->validated())); return redirect()->route('cms.case-studies.edit', $study)->with('status', 'Case study created.'); }
    public function edit(Request $request, CmsCaseStudyRepository $studies, int $caseStudy): View { return view('command-center.cms.case-studies.edit', ['study' => $studies->findForCompany($request->user()->company_id, $caseStudy), 'media' => CmsMedia::query()->where('company_id', $request->user()->company_id)->orderBy('file_name')->get()]); }
    public function update(CmsCaseStudyRequest $request, CmsCaseStudyRepository $studies, CmsCaseStudyService $service, int $caseStudy): RedirectResponse { $study = $service->update($studies->findForCompany($request->user()->company_id, $caseStudy), $request->user(), $this->payload($request->validated())); return redirect()->route('cms.case-studies.edit', $study)->with('status', 'Case study updated.'); }
