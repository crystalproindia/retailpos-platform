...
class CmsCaseStudyController extends Controller
{
    ...
    public function preview(Request $request, CmsCaseStudyRepository $studies, CmsPreviewService $previews, int $caseStudy): RedirectResponse { $link = $previews->create($studies->findForCompany($request->user()->company_id, $caseStudy), $request->user()); return back()->with('preview_url', $link['url'])->with('status', 'Draft preview link created. It expires in 30 minutes.'); }
    public function revokePreview(Request $request, CmsCaseStudyRepository $studies, CmsPreviewService $previews, int $caseStudy): RedirectResponse { $previews->revoke($studies->findForCompany($request->user()->company_id, $caseStudy), $request->user()); return back()->with('status', 'Draft preview links revoked.'); }
