...
class InvoiceController extends Controller
{
    ...
    public function show(Request $request, InvoiceRepository $invoices, InvoiceService $service, InvoiceReminderSettingsService $reminderSettings, int $invoice): View { $record = $service->refreshStatus($invoices->find($request->user(), $invoice)); $setting = $reminderSettings->ensure($request->user()->company); return view('command-center.crm.invoices.show', ['invoice' => $record->load(['items', 'payments.recorder', 'quotation', 'lead', 'latestInvoiceEmailDelivery', 'invoiceEmailDeliveries', 'reminderEmailDeliveries.createdBy']), 'reminderRules' => $setting->rules->where('enabled', true)]); }
    ...
    public function print(Request $request, InvoiceRepository $invoices, InvoicePdfService $pdf, int $invoice): Response { $record=$invoices->find($request->user(),$invoice); return $pdf->document($record)->stream($pdf->filename($record)); }
    ...
    public function send(Request $request, InvoiceRepository $invoices, InvoiceShareService $sharing, int $invoice): RedirectResponse { $result=$sharing->send($invoices->find($request->user(),$invoice),$request->user(),(string)$request->validate(['email'=>['required','email']])['email'], attachInvoicePdf: true); return back()->with($result['configured']?'status':'error',$result['configured']?'Invoice email with PDF attachment queued.':'Invoice saved; email skipped because SMTP is not configured.'); }
    public function resend(Request $request, InvoiceRepository $invoices, EmailDeliveryService $email, int $invoice, int $delivery): RedirectResponse { $record = $invoices->find($request->user(), $invoice); $email->manualResend($record->invoiceEmailDeliveries()->findOrFail($delivery), $request->user()); return back()->with('status', 'Invoice email queued for resend with its PDF attachment.'); }
    ...
    public function reminder(SendInvoiceReminderRequest $request, InvoiceRepository $invoices, InvoiceReminderService $reminders, int $invoice): RedirectResponse { $data = $request->validated(); $result = $reminders->queueManual($invoices->find($request->user(), $invoice), $request->user(), InvoiceReminderStage::from($data['stage']), $request->boolean('attach_pdf'), $data['note'] ?? null); return back()->with($result['configured'] ? 'status' : 'error', $result['configured'] ? 'Payment reminder queued for delivery.' : 'Reminder was recorded, but SMTP is not configured.'); }
