...
class CmsWebsiteControlRepository
{
    ...
    public function counts(int $companyId): array
    {
        return [
            ...
            'published_case_studies' => CmsCaseStudy::query()->where('company_id', $companyId)->where('status', 'published')->count(), 'draft_case_studies' => CmsCaseStudy::query()->where('company_id', $companyId)->where('status', 'draft')->count(),
