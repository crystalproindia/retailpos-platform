...
class LeadRepository
{
    ...
    public function findForUser(User $user, int $leadId, bool $withTrashed = false): CrmLead
    {
        return $this->queryForUser($user, $withTrashed)
            ->with(['activities.assignedUser', 'notes.user', 'auditLogs.user', 'tags', 'crmCompany', 'contact', 'demoSchedules.assignedTo', 'demoSchedules.scheduledBy', 'quotations.items', 'quotations.creator', 'crmCustomer.primaryContact', 'latestQuotation', 'latestProforma', 'latestActivity', 'leadScore'])
            ->findOrFail($leadId);
    }
