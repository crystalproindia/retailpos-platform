...
class CmsCaseStudyService
{
    ...
    public function publish(CmsCaseStudy $study, User $user): CmsCaseStudy { $study->update(['status' => 'published', 'published_at' => now()]); $this->auditLogger->record('cms.case_study.published', $study, 'Case study published'); $this->events->dispatch('cms.case_study.published', $user, $study, ['title' => $study->title]); return $study->refresh(); }
    public function unpublish(CmsCaseStudy $study, User $user): CmsCaseStudy { $study->update(['status' => 'draft', 'published_at' => null]); $this->auditLogger->record('cms.case_study.unpublished', $study, 'Case study unpublished'); $this->events->dispatch('cms.case_study.unpublished', $user, $study, ['title' => $study->title]); return $study->refresh(); }
    ...
    /** @param array<string, mixed> $data @return array<string, mixed> */ private function payload(array $data): array { $payload = collect($data)->except(['slug', 'sections'])->all(); $payload['metrics'] = array_filter($data['metrics'] ?? []); $payload['gallery_media_ids'] = array_values(array_filter($data['gallery_media_ids'] ?? [])); if (($payload['status'] ?? 'draft') === 'published') $payload['published_at'] ??= now(); return $payload; }
