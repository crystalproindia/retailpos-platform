...
class PosOfflineSyncService
{
    ...
    {
        $products = Product::query()->with(['category', 'brand', 'stockLevels' => fn ($query) => $query->where('branch_id', $user->branch_id)])->where('company_id', $user->company_id)->where('is_active', true)->where('status', Product::STATUS_ACTIVE)->orderBy('name')->limit(500)->get()->map(fn (Product $product) => ['id' => $product->id, 'name' => $product->name, 'sku' => $product->sku, 'barcode' => $product->barcode, 'price' => (float) $product->selling_price, 'category' => $product->category?->name, 'category_id' => $product->category_id, 'brand' => $product->brand?->name, 'image' => $product->imageUrl(true), 'track_inventory' => (bool) $product->track_inventory, 'available_stock' => (float) $product->stockLevels->sum('quantity_available')]);
