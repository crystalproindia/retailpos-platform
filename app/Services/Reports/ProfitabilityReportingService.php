...
class ProfitabilityReportingService
{
    ...
    {
        $allocations = DB::table('crm_invoice_amendment_allocations')
            ->selectRaw('invoice_item_id, COALESCE(SUM(taxable_discount), 0) as taxable_discount')
            ->groupBy('invoice_item_id');
        $query = DB::table('crm_invoice_items as item')
            ->join('crm_invoices as invoice', 'invoice.id', '=', 'item.invoice_id')
            ->leftJoinSub($allocations, 'overall_discount', 'overall_discount.invoice_item_id', '=', 'item.id')
