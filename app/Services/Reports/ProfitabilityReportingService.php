...
class ProfitabilityReportingService
{
    ...
    private function posReturns(User $user, array $scope, array $range, array $context): Builder
    {
        ...
        return $query->selectRaw("'pos' as source, 'return' as entry_type, item.company_id, item.pos_sale_id as document_id, ".$this->concat(["'pos:'", 'item.pos_sale_id'])." as document_key, sale.sale_number as document_number, return_document.completed_at as document_date, COALESCE(sale.customer_name_snapshot, 'Walk-in') as customer_name, sale.branch_id as outlet_id, COALESCE(outlet.name, 'Unassigned') as outlet_name, sale.completed_by as salesperson_id, COALESCE(salesperson.name, 'Unassigned') as salesperson_name, item.product_id, item.product_name, item.sku, item.category_id, COALESCE(item.category_name_snapshot, 'Uncategorized') as category_name, item.brand_id_snapshot as brand_id, COALESCE(item.brand_name_snapshot, 'Unbranded') as brand_name, 0 as quantity_sold, -return_item.return_quantity as quantity_returned, -return_item.return_quantity as net_quantity, -return_item.gross_adjustment as gross_sales, -return_item.discount_adjustment as discounts, -return_item.taxable_adjustment as return_sales, -return_item.taxable_adjustment as net_sales, item.unit_cost_snapshot, CASE WHEN item.cost_snapshot_status IN ('captured', 'reconstructed') THEN -(return_item.return_quantity * item.unit_cost_snapshot) ELSE 0 END as cogs, CASE WHEN item.cost_snapshot_status IN ('captured', 'reconstructed') THEN -return_item.taxable_adjustment + (return_item.return_quantity * item.unit_cost_snapshot) ELSE 0 END as gross_profit, COALESCE(item.cost_snapshot_status, 'unavailable') as cost_status, COALESCE(item.cost_snapshot_method, 'unavailable') as cost_provenance");
    }
