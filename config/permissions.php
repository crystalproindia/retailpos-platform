<?php

$crmManagementRoles = ['administrator', 'manager'];
$crmUserRoles = ['administrator', 'manager', 'sales'];
$notificationManagementRoles = ['administrator', 'manager'];
$notificationUserRoles = ['administrator', 'manager', 'sales'];
$operationsManagementRoles = ['administrator', 'manager'];
$inventoryManagementRoles = ['administrator', 'manager'];
$inventoryUserRoles = ['administrator', 'manager', 'sales'];
$purchaseManagementRoles = ['administrator', 'manager'];
$promotionManagementRoles = ['administrator', 'manager'];
$promotionReadRoles = ['administrator', 'manager', 'sales'];
$administratorRoles = ['administrator'];
$cmsManagementRoles = ['administrator', 'manager'];
$customerManagementRoles = ['administrator', 'manager'];
$customerUserRoles = ['administrator', 'manager', 'sales'];
$posUserRoles = ['administrator', 'manager', 'sales'];
$posManagementRoles = ['administrator', 'manager'];
$managementRoles = ['administrator', 'manager'];
$taskUserRoles = ['administrator', 'manager', 'sales', 'staff'];
$taskManagementRoles = ['administrator', 'manager'];
$attendanceUserRoles = ['administrator', 'manager', 'sales', 'staff'];
$attendanceManagementRoles = ['administrator', 'manager'];

return [
    'capabilities' => [
        'tasks.view' => $taskUserRoles,
        'tasks.create' => $taskUserRoles,
        'tasks.create_work' => $taskUserRoles,
        'tasks.update_own' => $taskUserRoles,
        'tasks.archive' => $taskUserRoles,
        'tasks.view_team' => $taskManagementRoles,
        'tasks.manage_team' => $taskManagementRoles,
        'tasks.assign' => $taskManagementRoles,
        'tasks.reassign' => $taskManagementRoles,
        'tasks.reopen' => $taskManagementRoles,
        'tasks.manage_recurring' => $taskUserRoles,
        'tasks.export' => $taskManagementRoles,
        'tasks.rules.manage' => $administratorRoles,
        'workforce.view' => ['administrator', 'manager'],
        'workforce.manage' => ['administrator'],
        'workforce.self.view' => ['administrator', 'manager', 'sales', 'staff'],
        'workforce.reviews.manage' => ['administrator', 'manager'],
        'workforce.recognition.manage' => ['administrator', 'manager'],
        'workforce.export' => ['administrator', 'manager'],
        'attendance.view_own' => $attendanceUserRoles,
        'attendance.check_in' => $attendanceUserRoles,
        'attendance.check_out' => $attendanceUserRoles,
        'attendance.view_team' => $attendanceManagementRoles,
        'attendance.manage_team' => $attendanceManagementRoles,
        'attendance.correct_own' => $attendanceUserRoles,
        'attendance.review_corrections' => $attendanceManagementRoles,
        'attendance.export' => $attendanceManagementRoles,
        'attendance.summary_export' => $attendanceManagementRoles,
        'shifts.view_own' => $attendanceUserRoles,
        'shifts.view_team' => $attendanceManagementRoles,
        'shifts.manage' => $attendanceManagementRoles,
        'rosters.manage' => $attendanceManagementRoles,
        'leave.view_own' => $attendanceUserRoles,
        'leave.request' => $attendanceUserRoles,
        'leave.view_team' => $attendanceManagementRoles,
        'leave.approve' => $attendanceManagementRoles,
        'leave.manage_policies' => $administratorRoles,
        'leave.adjust_balances' => $attendanceManagementRoles,
        'holidays.manage' => $attendanceManagementRoles,
        'overtime.review' => $attendanceManagementRoles,
        'store.setup.manage' => $administratorRoles,
        'outlets.view' => $posUserRoles,
        'outlets.manage' => $managementRoles,
        'outlets.assign' => $administratorRoles,
        'outlets.context.switch' => $posUserRoles,
        'compliance.gst.view' => $posManagementRoles,
        'compliance.gst.settings.manage' => $administratorRoles,
        'compliance.gst.reports.view' => $posManagementRoles,
        'compliance.gst.exports.create' => $posManagementRoles,
        'compliance.gst.periods.review' => $posManagementRoles,
        'compliance.gst.periods.lock' => $administratorRoles,
        'compliance.credit_notes.view' => $posManagementRoles,
        'compliance.credit_notes.create' => $posManagementRoles,
        'compliance.credit_notes.issue' => $administratorRoles,
        'compliance.debit_notes.view' => $posManagementRoles,
        'compliance.debit_notes.create' => $posManagementRoles,
        'compliance.debit_notes.issue' => $administratorRoles,
        'compliance.einvoice.validate' => $posManagementRoles,
        'compliance.einvoice.submit' => $administratorRoles,
        'compliance.ewaybill.validate' => $posManagementRoles,
        'compliance.ewaybill.submit' => $administratorRoles,
        'pos.view' => $posUserRoles,
        'pos.access' => $posUserRoles,
        'pos.billing.access' => $posUserRoles,
        'pos.sales.create' => $posUserRoles,
        'pos.sales.hold' => $posUserRoles,
        'pos.sales.complete' => $posUserRoles,
        'pos.sales.view' => $posUserRoles,
        'pos.sales.view_all' => $posManagementRoles,
        'pos.sales.resume' => $posUserRoles,
        'pos.checkout' => $posUserRoles,
        'pos.payments.record' => $posUserRoles,
        'pos.hold' => $posUserRoles,
        'pos.discount.apply' => $posUserRoles,
        'pos.discount.override' => $posManagementRoles,
        'pos.price.override' => $posManagementRoles,
        'pos.products.view' => $posUserRoles,
        'pos.customers.view' => $posUserRoles,
        'pos.customers.manage' => $posUserRoles,
        'pos.stock.view' => $posManagementRoles,
        'pos.customers.create' => $posUserRoles,
        'pos.receipts.view' => $posUserRoles,
        'pos.receipts.print' => $posUserRoles,
        'pos.receipts.reprint' => $posManagementRoles,
        'pos.offline.use' => $posUserRoles,
        'pos.offline.sync' => $posUserRoles,
        'pos.offline.monitor' => $posManagementRoles,
        'pos.offline.retry' => $posManagementRoles,
        'pos.offline.settings' => $posManagementRoles,
        'pos.sales.void' => $posManagementRoles,
        'pos.returns.view' => $posUserRoles,
        'pos.returns.initiate' => $posUserRoles,
        'pos.returns.create' => $posUserRoles,
        'pos.returns.approve' => $posManagementRoles,
        'pos.returns.complete' => $posManagementRoles,
        'pos.returns.cancel' => $posUserRoles,
        'pos.returns.override_window' => $posManagementRoles,
        'pos.returns.reprint' => $posManagementRoles,
        'pos.returns.settings.manage' => $posManagementRoles,
        'pos.refunds.record' => $posManagementRoles,
        'pos.refunds.cash' => $posManagementRoles,
        'pos.refunds.store_credit' => $posManagementRoles,
        'pos.exchanges.create' => $posUserRoles,
        'pos.return_receipts.print' => $posManagementRoles,
        'pos.return_receipts.reprint' => $posManagementRoles,
        'pos.registers.view' => $posUserRoles,
        'pos.registers.manage' => $posManagementRoles,
        'pos.sessions.open' => $posUserRoles,
        'pos.sessions.close' => $posManagementRoles,
        'pos.sales.export' => $posManagementRoles,
        'pos.products.manage' => $posManagementRoles,
        'pos.stock.opening' => $posManagementRoles,
        'pos.settings.manage' => $posManagementRoles,
        'pos.dashboard.view' => $posUserRoles,
        'customers.view' => $customerUserRoles,
        'customers.dashboard.view' => $customerUserRoles,
        'customers.create' => $customerUserRoles,
        'customers.update' => $customerUserRoles,
        'customers.delete' => $customerManagementRoles,
        'customers.restore' => $customerManagementRoles,
        'customers.groups.view' => $customerUserRoles,
        'customers.groups.manage' => $customerManagementRoles,
        'customers.loyalty.view' => $customerUserRoles,
        'customers.loyalty.adjust' => $customerManagementRoles,
        'customers.wallet.view' => $customerUserRoles,
        'customers.wallet.adjust' => $customerManagementRoles,
        'customers.insights.view' => $customerUserRoles,
        'customers.birthdays.view' => $customerUserRoles,
        'customers.inactive.view' => $customerUserRoles,
        'customers.lost.view' => $customerUserRoles,
        'customers.returns.view' => $customerUserRoles,
        'customers.settings.manage' => $customerManagementRoles,
        'cms.view' => $cmsManagementRoles,
        'cms.create' => $cmsManagementRoles,
        'cms.update' => $cmsManagementRoles,
        'cms.publish' => $cmsManagementRoles,
        'cms.delete' => $cmsManagementRoles,
        'cms.content.view' => $cmsManagementRoles,
        'cms.content.create' => $cmsManagementRoles,
        'cms.content.update' => $cmsManagementRoles,
        'cms.content.publish' => $cmsManagementRoles,
        'cms.content.delete' => $cmsManagementRoles,
        'cms.navigation.manage' => $cmsManagementRoles,
        'cms.website_builder.view' => $cmsManagementRoles,
        'cms.branding.manage' => $cmsManagementRoles,
        'cms.theme.manage' => $cmsManagementRoles,
        'cms.header.manage' => $cmsManagementRoles,
        'cms.footer.manage' => $cmsManagementRoles,
        'cms.pages.manage' => $cmsManagementRoles,
        'cms.homepage.manage' => $cmsManagementRoles,
        'cms.media.manage' => $cmsManagementRoles,
        'cms.seo.manage' => $cmsManagementRoles,
        'cms.redirects.manage' => $cmsManagementRoles,
        'cms.client_logos.manage' => $cmsManagementRoles,
        'cms.case_studies.manage' => $cmsManagementRoles,
        'cms.testimonials.manage' => $cmsManagementRoles,
        'cms.trust_metrics.manage' => $cmsManagementRoles,
        'cms.faq.manage' => $cmsManagementRoles,
        'cms.cta.manage' => $cmsManagementRoles,
        'cms.settings.manage' => $cmsManagementRoles,
        'website.pages.view' => $cmsManagementRoles,
        'website.pages.create' => $cmsManagementRoles,
        'website.pages.update' => $cmsManagementRoles,
        'website.pages.delete' => $cmsManagementRoles,
        'website.pages.publish' => $cmsManagementRoles,
        'website.sections.manage' => $cmsManagementRoles,
        'website.case_studies.view' => $cmsManagementRoles,
        'website.case_studies.create' => $cmsManagementRoles,
        'website.case_studies.update' => $cmsManagementRoles,
        'website.case_studies.publish' => $cmsManagementRoles,
        'website.case_studies.delete' => $cmsManagementRoles,
        'website.media.view' => $cmsManagementRoles,
        'website.media.upload' => $cmsManagementRoles,
        'website.media.delete' => $cmsManagementRoles,
        'website.navigation.view' => $cmsManagementRoles,
        'website.navigation.update' => $cmsManagementRoles,
        'website.settings.view' => $cmsManagementRoles,
        'website.settings.update' => $cmsManagementRoles,
        'website.seo.update' => $cmsManagementRoles,
        'website.revisions.view' => $cmsManagementRoles,
        'website.revisions.restore' => $cmsManagementRoles,
        'website.preview.create' => $cmsManagementRoles,
        'website.preview.revoke' => $cmsManagementRoles,
        'website.import.view' => $administratorRoles,
        'website.import.execute' => $administratorRoles,
        'crm.view' => $crmUserRoles,
        'crm.leads.view' => $crmUserRoles,
        'crm.leads.create' => $crmUserRoles,
        'crm.leads.update' => $crmUserRoles,
        'crm.leads.delete' => $crmManagementRoles,
        'crm.leads.assign' => $crmManagementRoles,
        'crm.leads.convert' => $crmUserRoles,
        'crm.companies.manage' => $crmUserRoles,
        'crm.contacts.manage' => $crmUserRoles,
        'crm.activities.manage' => $crmUserRoles,
        'crm.pipeline.manage' => $crmUserRoles,
        'crm.ai.view' => $crmUserRoles,
        'crm.ai.generate' => $crmUserRoles,
        'crm.ai.refresh_score' => $crmUserRoles,
        'ai.dashboard.view' => $crmManagementRoles,
        'ai.forecasts.view' => $crmManagementRoles,
        'ai.forecasts.run' => $administratorRoles,
        'ai.insights.review' => $crmManagementRoles,
        'ai.settings.manage' => $administratorRoles,
        'ai.exports.create' => $crmManagementRoles,
        'crm.settings.manage' => $crmManagementRoles,
        'crm.demos.view' => $crmUserRoles,
        'crm.demos.create' => $crmUserRoles,
        'crm.demos.update' => $crmUserRoles,
        'crm.demos.cancel' => $crmUserRoles,
        'crm.demos.complete' => $crmUserRoles,
        'crm.quotations.view' => $crmUserRoles,
        'crm.quotations.create' => $crmUserRoles,
        'crm.quotations.update' => $crmUserRoles,
        'crm.quotations.send' => $crmUserRoles,
        'crm.quotations.accept' => $crmUserRoles,
        'crm.quotations.reject' => $crmUserRoles,
        'crm.quotations.delete' => $crmManagementRoles,
        'sales.pipeline.view' => $crmUserRoles,
        'sales.pipeline.manage' => $crmUserRoles,
        'sales.leads.assign' => $crmManagementRoles,
        'sales.activities.create' => $crmUserRoles,
        'sales.followups.view' => $crmUserRoles,
        'sales.followups.manage' => $crmUserRoles,
        'sales.opportunities.view' => $crmUserRoles,
        'sales.opportunities.create' => $crmUserRoles,
        'sales.opportunities.update' => $crmUserRoles,
        'sales.opportunities.close' => $crmManagementRoles,
        'sales.quotations.view' => $crmUserRoles,
        'sales.quotations.create' => $crmUserRoles,
        'sales.quotations.update' => $crmUserRoles,
        'sales.quotations.send' => $crmUserRoles,
        'sales.quotations.accept' => $crmUserRoles,
        'sales.quotations.cancel' => $crmManagementRoles,
        'sales.quotations.pdf' => $crmUserRoles,
        'sales.quotations.public_link' => $crmUserRoles,
        'sales.invoices.view' => $crmUserRoles,
        'sales.invoices.create' => $crmUserRoles,
        'sales.invoices.update' => $crmManagementRoles,
        'sales.invoices.amend' => $crmManagementRoles,
        'sales.invoices.issue' => $crmUserRoles,
        'sales.invoices.send' => $crmUserRoles,
        'sales.invoices.cancel' => $crmManagementRoles,
        'sales.invoices.pdf' => $crmUserRoles,
        'sales.invoices.public_link' => $crmUserRoles,
        'sales.returns.view' => $crmUserRoles,
        'sales.returns.create' => $crmManagementRoles,
        'sales.returns.finalize' => $crmManagementRoles,
        'sales.credit_notes.pdf' => $crmUserRoles,
        'sales.payments.view' => $crmUserRoles,
        'sales.payments.record' => $crmUserRoles,
        'sales.payments.clear' => $crmManagementRoles,
        'sales.payments.reverse' => $crmManagementRoles,
        'sales.receipts.view' => $crmUserRoles,
        'sales.receipts.pdf' => $crmUserRoles,
        'sales.receipts.send' => $crmUserRoles,
        'sales.reminders.send' => $crmUserRoles,
        'sales.reminders.manage' => $crmManagementRoles,
        'sales.finance.dashboard' => $crmUserRoles,
        'sales.finance.export' => $crmManagementRoles,
        'finance.receivables.view' => $crmManagementRoles,
        'finance.payables.view' => $purchaseManagementRoles,
        'finance.statements.view' => $managementRoles,
        'finance.payments.allocate' => $managementRoles,
        'finance.credit-limits.manage' => $managementRoles,
        'finance.credit-limits.override' => $managementRoles,
        'finance.reconciliation.manage' => $managementRoles,
        'finance.exports' => $managementRoles,
        'finance.expenses.view' => $managementRoles,
        'finance.expenses.create' => $managementRoles,
        'finance.expenses.update_draft' => $managementRoles,
        'finance.expenses.post' => $managementRoles,
        'finance.expenses.reverse' => $managementRoles,
        'finance.expense_categories.manage' => $managementRoles,
        'finance.profit_and_loss.view' => $managementRoles,
        'finance.profit_and_loss.export' => $managementRoles,
        'sales.invoice_settings.manage' => $administratorRoles,
        'crm.customers.view' => $crmUserRoles,
        'crm.customers.create' => $crmUserRoles,
        'crm.customers.update' => $crmUserRoles,
        'crm.customers.convert' => $crmUserRoles,
        'crm.customers.delete' => $crmManagementRoles,
        'crm.customers.portal.manage' => $crmManagementRoles,
        'crm.proformas.view' => $crmUserRoles,
        'crm.proformas.create' => $crmUserRoles,
        'crm.proformas.update' => $crmUserRoles,
        'crm.proformas.send' => $crmUserRoles,
        'crm.proformas.record_payment' => $crmUserRoles,
        'crm.proformas.cancel' => $crmManagementRoles,
        'crm.proformas.delete' => $crmManagementRoles,
        'crm.onboarding.view' => $crmUserRoles,
        'crm.onboarding.create' => $crmUserRoles,
        'crm.onboarding.update' => $crmUserRoles,
        'crm.onboarding.assign' => $crmManagementRoles,
        'crm.onboarding.complete_task' => $crmUserRoles,
        'crm.onboarding.manage_documents' => $crmUserRoles,
        'crm.onboarding.cancel' => $crmManagementRoles,
        'crm.support.view' => $crmUserRoles,
        'crm.support.create' => $crmUserRoles,
        'crm.support.update' => $crmManagementRoles,
        'crm.support.assign' => $crmManagementRoles,
        'crm.support.reply' => $crmManagementRoles,
        'crm.support.resolve' => $crmManagementRoles,
        'crm.support.close' => $crmManagementRoles,
        'crm.support.delete' => $crmManagementRoles,
        'crm.reports.view' => $crmUserRoles,
        'crm.reports.executive' => $crmManagementRoles,
        'crm.reports.export' => $crmManagementRoles,
        'reports.profitability.view' => $crmManagementRoles,
        'reports.profitability.export' => $crmManagementRoles,
        'integrations.email.view' => $notificationManagementRoles,
        'integrations.email.manage' => $administratorRoles,
        'email.deliveries.view' => $notificationManagementRoles,
        'email.deliveries.retry' => $notificationManagementRoles,
        'email.test.send' => $administratorRoles,
        'notifications.view' => $notificationUserRoles,
        'notifications.manage_own' => $notificationUserRoles,
        'notifications.preferences.manage_own' => $notificationUserRoles,
        'notifications.preferences.manage_company' => $notificationManagementRoles,
        'notifications.events.view' => $notificationManagementRoles,
        'notifications.deliveries.view' => $notificationManagementRoles,
        'notifications.templates.manage' => $administratorRoles,
        'notifications.webhooks.view' => $notificationManagementRoles,
        'notifications.webhooks.manage' => $administratorRoles,
        'notifications.webhooks.retry' => $notificationManagementRoles,
        'notifications.settings.manage' => $administratorRoles,
        'automation.manage' => $administratorRoles,
        'operations.view' => $operationsManagementRoles,
        'operations.health.view' => $operationsManagementRoles,
        'operations.queue.view' => $operationsManagementRoles,
        'operations.failed_jobs.view' => $operationsManagementRoles,
        'operations.failed_jobs.retry' => $administratorRoles,
        'operations.failed_jobs.delete' => $administratorRoles,
        'operations.schedule.view' => $operationsManagementRoles,
        'operations.application.view' => $operationsManagementRoles,
        'operations.settings.manage' => $administratorRoles,
        'inventory.view' => $inventoryUserRoles,
        'inventory.products.view' => $inventoryUserRoles,
        'inventory.products.create' => $inventoryManagementRoles,
        'inventory.products.update' => $inventoryManagementRoles,
        'inventory.products.image.manage' => $inventoryManagementRoles,
        'inventory.products.delete' => $inventoryManagementRoles,
        'inventory.products.restore' => $inventoryManagementRoles,
        'inventory.categories.manage' => $inventoryManagementRoles,
        'inventory.brands.manage' => $inventoryManagementRoles,
        'inventory.units.manage' => $inventoryManagementRoles,
        'inventory.tax.manage' => $inventoryManagementRoles,
        'inventory.warehouses.manage' => $inventoryManagementRoles,
        'inventory.stock.view' => $inventoryManagementRoles,
        'inventory.stock.view_all_locations' => $administratorRoles,
        'inventory.stock.opening' => $inventoryManagementRoles,
        'inventory.stock.adjust' => $inventoryManagementRoles,
        'inventory.stock.approve_adjustment' => $inventoryManagementRoles,
        'inventory.adjustments.view' => $inventoryManagementRoles,
        'inventory.adjustments.create' => $inventoryManagementRoles,
        'inventory.adjustments.approve' => $inventoryManagementRoles,
        'inventory.transfers.view' => $inventoryManagementRoles,
        'inventory.transfers.create' => $inventoryManagementRoles,
        'inventory.transfers.approve' => $inventoryManagementRoles,
        'inventory.transfers.pack' => $inventoryManagementRoles,
        'inventory.transfers.dispatch' => $inventoryManagementRoles,
        'inventory.transfers.receive' => $inventoryManagementRoles,
        'inventory.transfers.cancel' => $inventoryManagementRoles,
        'inventory.transfers.resolve_discrepancy' => $inventoryManagementRoles,
        'inventory.counts.view' => $inventoryManagementRoles,
        'inventory.counts.create' => $inventoryManagementRoles,
        'inventory.counts.submit' => $inventoryManagementRoles,
        'inventory.counts.approve' => $inventoryManagementRoles,
        'inventory.counts.post' => $inventoryManagementRoles,
        'inventory.batch.view' => $inventoryManagementRoles,
        'inventory.batch.manage' => $inventoryManagementRoles,
        'inventory.serial.view' => $inventoryManagementRoles,
        'inventory.serial.manage' => $inventoryManagementRoles,
        'inventory.labels.print' => $inventoryManagementRoles,
        'inventory.reports.export' => $inventoryManagementRoles,
        'inventory.barcode.manage' => $inventoryManagementRoles,
        'inventory.barcode.print' => $inventoryManagementRoles,
        'inventory.reorder.view' => $inventoryManagementRoles,
        'inventory.reorder.manage' => $inventoryManagementRoles,
        'inventory.channels.view' => $inventoryManagementRoles,
        'inventory.channels.manage' => $inventoryManagementRoles,
        'inventory.settings.manage' => $inventoryManagementRoles,
        'inventory.decision_dashboard.view' => $inventoryManagementRoles,
        'purchases.view' => $purchaseManagementRoles,
        'purchases.dashboard.view' => $purchaseManagementRoles,
        'purchases.supplier_dashboard.view' => $purchaseManagementRoles,
        'purchases.suppliers.view' => $purchaseManagementRoles,
        'purchases.suppliers.create' => $purchaseManagementRoles,
        'purchases.suppliers.update' => $purchaseManagementRoles,
        'purchases.suppliers.delete' => $purchaseManagementRoles,
        'purchases.suppliers.restore' => $purchaseManagementRoles,
        'purchases.supplier_products.manage' => $purchaseManagementRoles,
        'purchases.supplier_scores.view' => $purchaseManagementRoles,
        'purchases.supplier_scores.manage' => $purchaseManagementRoles,
        'purchases.requests.view' => $purchaseManagementRoles,
        'purchases.requests.create' => $purchaseManagementRoles,
        'purchases.requests.update' => $purchaseManagementRoles,
        'purchases.requests.approve' => $purchaseManagementRoles,
        'purchases.requests.reject' => $purchaseManagementRoles,
        'purchases.requests.convert' => $purchaseManagementRoles,
        'purchases.orders.view' => $purchaseManagementRoles,
        'purchases.orders.create' => $purchaseManagementRoles,
        'purchases.orders.update' => $purchaseManagementRoles,
        'purchases.orders.approve' => $purchaseManagementRoles,
        'purchases.orders.send' => $purchaseManagementRoles,
        'purchases.orders.cancel' => $purchaseManagementRoles,
        'purchases.grn.view' => $purchaseManagementRoles,
        'purchases.grn.create' => $purchaseManagementRoles,
        'purchases.grn.receive' => $purchaseManagementRoles,
        'purchases.returns.view' => $purchaseManagementRoles,
        'purchases.returns.create' => $purchaseManagementRoles,
        'purchases.returns.approve' => $purchaseManagementRoles,
        'purchases.returns.complete' => $purchaseManagementRoles,
        'purchases.settings.manage' => $purchaseManagementRoles,
        'purchase-invoices.view' => $purchaseManagementRoles,
        'purchase-invoices.create' => $purchaseManagementRoles,
        'purchase-invoices.update' => $purchaseManagementRoles,
        'purchase-invoices.verify' => $purchaseManagementRoles,
        'purchase-invoices.approve' => $purchaseManagementRoles,
        'purchase-invoices.cancel' => $purchaseManagementRoles,
        'purchase-invoices.override-quantity' => $administratorRoles,
        'purchase-invoices.override-variance' => $administratorRoles,
        'supplier-payments.view' => $purchaseManagementRoles,
        'supplier-payments.create' => $purchaseManagementRoles,
        'supplier-payments.approve' => $purchaseManagementRoles,
        'supplier-payments.reverse' => $administratorRoles,
        'purchase-reports.view' => $purchaseManagementRoles,
        'purchase-reports.export' => $purchaseManagementRoles,
        'input-gst-reports.view' => $purchaseManagementRoles,
        'input-gst-reports.export' => $purchaseManagementRoles,
        'promotions.view' => $promotionReadRoles,
        'promotions.dashboard.view' => $promotionReadRoles,
        'promotions.campaigns.view' => $promotionReadRoles,
        'promotions.campaigns.create' => $promotionManagementRoles,
        'promotions.campaigns.update' => $promotionManagementRoles,
        'promotions.campaigns.delete' => $promotionManagementRoles,
        'promotions.campaigns.restore' => $promotionManagementRoles,
        'promotions.rules.view' => $promotionReadRoles,
        'promotions.rules.create' => $promotionManagementRoles,
        'promotions.rules.update' => $promotionManagementRoles,
        'promotions.rules.delete' => $promotionManagementRoles,
        'promotions.rules.restore' => $promotionManagementRoles,
        'promotions.rules.approve' => $promotionManagementRoles,
        'promotions.rules.activate' => $promotionManagementRoles,
        'promotions.rules.pause' => $promotionManagementRoles,
        'promotions.coupons.view' => $promotionReadRoles,
        'promotions.coupons.create' => $promotionManagementRoles,
        'promotions.coupons.update' => $promotionManagementRoles,
        'promotions.coupons.disable' => $promotionManagementRoles,
        'promotions.simulator.view' => $promotionManagementRoles,
        'promotions.simulator.run' => $promotionManagementRoles,
        'promotions.usage.view' => $promotionManagementRoles,
        'promotions.settings.manage' => $promotionManagementRoles,
        'saas.dashboard.view' => $administratorRoles,
        'saas.tenants.view' => $administratorRoles,
        'saas.tenants.create' => $administratorRoles,
        'saas.tenants.update' => $administratorRoles,
        'saas.tenants.suspend' => $administratorRoles,
        'saas.tenants.reactivate' => $administratorRoles,
        'saas.plans.view' => $administratorRoles,
        'saas.plans.create' => $administratorRoles,
        'saas.plans.update' => $administratorRoles,
        'saas.plans.archive' => $administratorRoles,
        'saas.subscriptions.view' => $administratorRoles,
        'saas.subscriptions.create' => $administratorRoles,
        'saas.subscriptions.update' => $administratorRoles,
        'saas.subscriptions.renew' => $administratorRoles,
        'saas.subscriptions.cancel' => $administratorRoles,
        'saas.trials.extend' => $administratorRoles,
        'saas.usage.view' => $administratorRoles,
        'saas.overrides.manage' => $administratorRoles,
        'saas.onboarding.manage' => $administratorRoles,
        'saas.resellers.view' => $administratorRoles,
        'saas.resellers.manage' => $administratorRoles,
        'saas.billing.view' => $administratorRoles,
        'saas.billing.issue' => $administratorRoles,
        'saas.billing.void' => $administratorRoles,
        'saas.billing.record-payment' => $administratorRoles,
        'saas.billing.refund' => $administratorRoles,
        'saas.billing.reconcile' => $administratorRoles,
        'saas.billing.gateway.manage' => $administratorRoles,
        'subscription.billing.view' => $administratorRoles,
        'subscription.billing.pay' => $administratorRoles,
        'subscription.billing.receipts.view' => $administratorRoles,
        'subscription.view' => $administratorRoles,
        'subscription.manage' => $administratorRoles,
        'subscription.request-plan-change' => $administratorRoles,
        'subscription.cancel' => $administratorRoles,
        'subscription.billing-contact.update' => $administratorRoles,
        'company.profile.update' => $managementRoles,
        'subscription.usage.view' => $administratorRoles,
        'white-label.view' => $administratorRoles,
        'white-label.update' => $administratorRoles,
    ],
];
