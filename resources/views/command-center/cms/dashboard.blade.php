    ...
    <div class="space-y-6">
        ...
        <div class="grid gap-6 xl:grid-cols-[1.25fr_0.75fr]">
            <section class="cms-panel">
                ...
                <div class="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-5">@foreach(['Pages' => $contentEditor['pages'], 'Published' => $contentEditor['published'], 'Drafts' => $contentEditor['drafts'], 'Hidden sections' => $contentEditor['disabled_sections'], 'Active links' => $contentEditor['active_navigation_items']] as $label => $value)<div class="cms-subtle-panel"><p class="text-xs font-medium text-slate-500">{{ $label }}</p><p class="mt-1 text-2xl font-semibold text-slate-900">{{ $value }}</p></div>@endforeach</div>
                @if ($contentEditor['recent']->isNotEmpty())<div class="mt-4 border-t border-slate-100 pt-4"><p class="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">Recently updated</p><div class="mt-3 flex flex-wrap gap-2">@foreach($contentEditor['recent'] as $page)<a href="{{ route('cms.content.pages.show', $page) }}" class="rounded-lg border border-slate-200 px-3 py-2 text-sm font-semibold text-slate-700 hover:border-teal-200 hover:bg-teal-50 hover:text-teal-800">{{ $page->title }}</a>@endforeach</div></div>@endif
            </section>
            <section class="cms-panel">
