    ...
    <div class="space-y-6">
        ...
        <section class="grid gap-4 xl:grid-cols-[0.9fr_1.1fr]">
            <article class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                ...
                <div class="mt-5 grid gap-3 sm:grid-cols-2"><div class="rounded-lg bg-rose-50 p-4 dark:bg-rose-950/20"><p class="text-sm text-rose-800 dark:text-rose-200">Hot leads</p><p class="mt-2 text-2xl font-semibold text-rose-950 dark:text-white">{{ $aiInsights['hot_count'] }}</p></div><div class="rounded-lg bg-amber-50 p-4 dark:bg-amber-950/20"><p class="text-sm text-amber-800 dark:text-amber-200">At-risk leads</p><p class="mt-2 text-2xl font-semibold text-amber-950 dark:text-white">{{ $aiInsights['at_risk_count'] }}</p></div><div class="rounded-lg bg-sky-50 p-4 dark:bg-sky-950/20"><p class="text-sm text-sky-800 dark:text-sky-200">Follow-ups today</p><p class="mt-2 text-2xl font-semibold text-sky-950 dark:text-white">{{ $aiInsights['follow_ups_today'] }}</p></div><div class="rounded-lg bg-slate-100 p-4 dark:bg-slate-950"><p class="text-sm text-slate-700 dark:text-slate-200">Hot overdue follow-ups</p><p class="mt-2 text-2xl font-semibold text-slate-950 dark:text-white">{{ $aiInsights['overdue_follow_ups'] }}</p></div></div>
            </article>
            <article class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                <h2 class="text-base font-semibold text-slate-950 dark:text-white">Highest priority leads</h2>
                <div class="mt-4 space-y-3">@forelse($aiInsights['hot_leads'] as $score)<a href="{{ route('crm.leads.show', $score->lead) }}" class="flex items-center justify-between gap-3 rounded-lg border border-slate-200 px-4 py-3 hover:bg-slate-50 dark:border-slate-800 dark:hover:bg-slate-800"><div><p class="font-semibold text-slate-950 dark:text-white">{{ $score->lead->title }}</p><p class="mt-1 text-xs text-slate-500 dark:text-slate-400">{{ $score->next_best_action }}</p></div><span class="rounded-full bg-rose-100 px-2.5 py-1 text-xs font-semibold text-rose-800 dark:bg-rose-950 dark:text-rose-200">{{ $score->score }}/100</span></a>@empty<p class="rounded-lg border border-dashed border-slate-300 px-4 py-6 text-center text-sm text-slate-500 dark:border-slate-700 dark:text-slate-400">No hot leads need attention right now.</p>@endforelse</div>
            </article>
        </section>
