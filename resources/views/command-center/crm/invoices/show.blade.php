...
@section('content')
    <div class="space-y-6">
        ...
        <section class="rounded-lg border border-sky-200 bg-white p-5 shadow-sm dark:border-sky-900 dark:bg-slate-900">
            <div class="flex flex-wrap items-start justify-between gap-3"><div><p class="text-xs font-semibold uppercase tracking-wide text-sky-700 dark:text-sky-300">Authoritative finance</p><h2 class="mt-1 text-lg font-semibold text-slate-950 dark:text-white">Payment &amp; Receivable Summary</h2></div><span class="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700 dark:bg-slate-800 dark:text-slate-200">{{ $receivable['payment_status'] }}</span></div>
            <div class="mt-5 grid gap-px overflow-hidden rounded-lg border border-slate-200 bg-slate-200 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 dark:border-slate-800 dark:bg-slate-800">
                @foreach(['Invoice Date' => $receivable['invoice_date']?->format('d M Y') ?? '—', 'Due Date' => $receivable['due_date']?->format('d M Y') ?? '—', 'Invoice Total' => $invoice->currency.' '.number_format($receivable['invoice_total']/100, 2), 'Amount Received' => $invoice->currency.' '.number_format($receivable['amount_received']/100, 2), 'Balance Receivable' => $invoice->currency.' '.number_format($receivable['balance_receivable']/100, 2), 'Last Payment' => $receivable['last_payment_date']?->format('d M Y') ?? '—'] as $label => $value)
                    <div @class(['bg-white p-4 dark:bg-slate-900', 'bg-sky-50 dark:bg-sky-950/30' => $label === 'Balance Receivable'])><p class="text-xs font-medium text-slate-500 dark:text-slate-400">{{ $label }}</p><p @class(['mt-1 text-sm font-semibold text-slate-950 dark:text-white', 'text-lg text-sky-800 dark:text-sky-200' => $label === 'Balance Receivable'])>{{ $value }}</p></div>
                @endforeach
            </div>
        </section>
        ...
        <section class="grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_360px]">
            ...
            <aside class="space-y-6">
                <article class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                    <div class="flex items-start justify-between gap-3"><div><h2 class="font-semibold text-slate-950 dark:text-white">Record payment</h2><p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Balance receivable <span class="font-semibold text-sky-700 dark:text-sky-300">{{ $invoice->currency }} {{ number_format($receivable['balance_receivable']/100,2) }}</span></p></div></div>
                    @if (! $invoice->status?->isEditable() && ! $invoice->status?->isTerminal() && $invoice->balance_due > 0)
                        <form method="POST" action="{{ route('sales.invoices.payments.store', $invoice) }}" class="mt-4 space-y-4">@csrf
                            <label class="block text-sm font-medium text-slate-700 dark:text-slate-200">Amount<input name="amount" type="number" step="0.01" min="0.01" max="{{ $invoice->balance_due }}" required value="{{ old('amount') }}" class="mt-1.5 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-slate-950 dark:border-slate-700 dark:bg-slate-950 dark:text-white">@error('amount')<span class="mt-1 block text-xs text-rose-600 dark:text-rose-300">{{ $message }}</span>@enderror</label>
                            <div class="grid gap-3 sm:grid-cols-2"><label class="block text-sm font-medium text-slate-700 dark:text-slate-200">Currency<input name="currency" value="{{ $invoice->currency }}" readonly class="mt-1.5 w-full rounded-lg border border-slate-300 bg-slate-50 px-3 py-2 dark:border-slate-700 dark:bg-slate-950 dark:text-white"></label><label class="block text-sm font-medium text-slate-700 dark:text-slate-200">Payment Date<input name="payment_date" type="date" value="{{ old('payment_date', today()->toDateString()) }}" class="mt-1.5 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 dark:border-slate-700 dark:bg-slate-950 dark:text-white"></label></div>
                            <label class="block text-sm font-medium text-slate-700 dark:text-slate-200">Payment Method<select name="payment_method" class="mt-1.5 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 dark:border-slate-700 dark:bg-slate-950 dark:text-white">@foreach (['bank_transfer','cash','cheque','card','upi','online','other'] as $method)<option value="{{ $method }}">{{ str($method)->replace('_', ' ')->headline() }}</option>@endforeach</select></label>
                            <label class="block text-sm font-medium text-slate-700 dark:text-slate-200">Reference / Note<input name="transaction_reference" value="{{ old('transaction_reference') }}" placeholder="Example: Advance Received" class="mt-1.5 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 dark:border-slate-700 dark:bg-slate-950 dark:text-white"></label>
