<form class="rounded-lg border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900" method="GET">
    <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-8">
        ...
        <label class="text-xs font-semibold text-slate-500 dark:text-slate-400">Lead status<select name="status_id" class="mt-1 block w-full rounded-lg border-slate-300 text-sm dark:border-slate-700 dark:bg-slate-950"><option value="">All statuses</option>@foreach($filterOptions['statuses'] as $status)<option value="{{ $status->id }}" @selected((string)request('status_id') === (string)$status->id)>{{ $status->name }}</option>@endforeach</select></label>
        <div class="flex items-end gap-2"><button class="w-full rounded-lg bg-slate-950 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-800 dark:bg-teal-300 dark:text-slate-950">Apply</button><a href="{{ url()->current() }}" class="rounded-lg border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-200">Reset</a></div>
    </div>
    <div class="mt-3 flex justify-end"><a href="{{ request()->fullUrl() }}" class="text-xs font-semibold text-teal-700 hover:text-teal-900 dark:text-teal-300">Refresh data</a></div>
</form>
