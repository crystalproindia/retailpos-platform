    ...
    <div class="space-y-6">
        ...
        @if ($taskMetrics)
            <section class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                <div class="flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><h2 class="text-base font-semibold text-slate-950 dark:text-white">My task focus</h2><p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Personal tasks stay private. Work tasks stay connected to the next operational action.</p></div><div class="flex items-center gap-4">@can('tasks.create')<a href="{{ route('tasks.index') }}#quick-add" class="text-sm font-semibold text-teal-700 hover:text-teal-900 dark:text-teal-300">Add task</a>@endcan<a href="{{ route('tasks.today') }}" class="text-sm font-semibold text-teal-700 hover:text-teal-900 dark:text-teal-300">Open My Day</a></div></div>
                <div class="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">@foreach([['label' => 'Due today', 'value' => $taskMetrics['work']['today'], 'route' => 'tasks.today', 'tone' => 'bg-sky-50 dark:bg-sky-950/30'], ['label' => 'Overdue work', 'value' => $taskMetrics['work']['overdue'], 'route' => 'tasks.overdue', 'tone' => 'bg-rose-50 dark:bg-rose-950/30'], ['label' => 'Upcoming work', 'value' => $taskMetrics['work']['upcoming'], 'route' => 'tasks.upcoming', 'tone' => 'bg-teal-50 dark:bg-teal-950/30'], ['label' => 'Personal tasks', 'value' => $taskMetrics['personal']['today'] + $taskMetrics['personal']['upcoming'] + $taskMetrics['personal']['overdue'], 'route' => 'tasks.personal', 'tone' => 'bg-violet-50 dark:bg-violet-950/30']] as $taskCard)<a href="{{ route($taskCard['route']) }}" class="rounded-lg border border-slate-200 p-4 transition hover:border-slate-300 hover:shadow-sm dark:border-slate-800 dark:hover:border-slate-700 {{ $taskCard['tone'] }}"><p class="text-sm font-medium text-slate-500 dark:text-slate-400">{{ $taskCard['label'] }}</p><p class="mt-2 text-2xl font-semibold text-slate-950 dark:text-white">{{ $taskCard['value'] }}</p></a>@endforeach</div>
            </section>
        @endif

        @if ($teamTaskMetrics)
            <section class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                <div class="flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><h2 class="text-base font-semibold text-slate-950 dark:text-white">Authorized team workload</h2><p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Only work tasks from outlets you can manage are included. Personal tasks are excluded.</p></div><a href="{{ route('tasks.team') }}" class="text-sm font-semibold text-teal-700 hover:text-teal-900 dark:text-teal-300">Open team tasks</a></div>
                <div class="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">@foreach([['label' => 'Due today', 'value' => $teamTaskMetrics['due_today'], 'route' => 'tasks.today', 'tone' => 'bg-sky-50 dark:bg-sky-950/30'], ['label' => 'Overdue', 'value' => $teamTaskMetrics['overdue'], 'route' => 'tasks.overdue', 'tone' => 'bg-rose-50 dark:bg-rose-950/30'], ['label' => 'Unassigned work', 'value' => $teamTaskMetrics['unassigned'], 'route' => 'tasks.team', 'tone' => 'bg-amber-50 dark:bg-amber-950/30'], ['label' => 'Completed today', 'value' => $teamTaskMetrics['completed_today'], 'route' => 'tasks.completed', 'tone' => 'bg-emerald-50 dark:bg-emerald-950/30']] as $teamCard)<a href="{{ route($teamCard['route']) }}" class="rounded-lg border border-slate-200 p-4 transition hover:border-slate-300 hover:shadow-sm dark:border-slate-800 dark:hover:border-slate-700 {{ $teamCard['tone'] }}"><p class="text-sm font-medium text-slate-500 dark:text-slate-400">{{ $teamCard['label'] }}</p><p class="mt-2 text-2xl font-semibold text-slate-950 dark:text-white">{{ $teamCard['value'] }}</p></a>@endforeach</div>
            </section>
        @endif
