        ...
        <section class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <div class="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                ...
                @can('inventory.products.update')
                    <div class="flex gap-2"><a href="{{ route('inventory.stock.product', $product) }}" class="inline-flex min-h-11 items-center rounded-lg border border-slate-300 px-4 text-sm font-semibold">Stock detail</a><a href="{{ route('inventory.products.edit', $product) }}" class="inline-flex min-h-11 items-center rounded-lg bg-slate-950 px-4 text-sm font-semibold text-white dark:bg-teal-300 dark:text-slate-950">Edit</a></div>
                @endcan
            </div>
            ...
            <div class="mt-4 space-y-3">
                @forelse ($product->stockLevels as $stock)
                    <div class="rounded-lg border border-slate-200 p-4 dark:border-slate-800">
                        ...
                        <div class="mt-2 grid grid-cols-4 gap-2 text-sm text-slate-500">
                            ...
                            <span>Damaged: <strong class="text-slate-900 dark:text-white">{{ $stock->quantity_damaged }}</strong></span>
                        </div>
                    </div>
                @empty
