    ...
    <div class="mx-auto grid max-w-7xl gap-6 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
        <section class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <form method="POST" action="{{ route('workforce.roles.store') }}" class="mt-5 space-y-4">@csrf
                ...
                <fieldset><legend class="text-sm font-semibold text-slate-700">Permission matrix</legend><p class="mt-1 text-xs text-slate-500">Permissions are checked on the server. Choose only capabilities this role needs.</p><div class="mt-3 max-h-80 space-y-2 overflow-y-auto rounded-lg border border-slate-200 p-3">@foreach($permissions as $permission => $roles)<label class="flex items-start gap-2 rounded p-1 text-sm text-slate-700 hover:bg-slate-50"><input type="checkbox" name="permissions[]" value="{{ $permission }}" @checked(in_array($permission, old('permissions', []))) class="mt-0.5 rounded border-slate-300 text-teal-600"><span><span class="font-medium">{{ str($permission)->replace('.', ' ')->headline() }}</span><span class="block text-xs text-slate-500">Available in the {{ collect($roles)->map(fn($role) => str($role)->headline())->join(', ') }} system role policy.</span></span></label>@endforeach</div></fieldset>
