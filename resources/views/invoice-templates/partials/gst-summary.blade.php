@if($render['is_gst'])<section class="gst-summary avoid-break">
    ...
    <table role="presentation">
        <thead>
            <tr><th>HSN/SAC</th><th>Treatment</th><th class="right">Taxable</th><th class="right">Rate</th><th class="right">CGST</th><th class="right">SGST</th><th class="right">IGST</th><th class="right">Cess</th><th class="right">Total tax</th></tr>
        </thead>
        <tbody>
            @foreach($render['tax_rows'] as $row)
                <tr>
                    <td>{{ $row['hsn_sac'] }}</td>
                    <td>{{ str($row['tax_treatment'])->replace('_', ' ')->headline() }}</td>
