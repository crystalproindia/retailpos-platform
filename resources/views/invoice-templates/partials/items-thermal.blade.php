<table class="thermal-items" role="presentation">
    ...
    <tbody>
        @foreach($invoice->items as $item)
            <tr>
                <td><strong>{{ $item->name }}</strong>@if($render['is_gst'] && $render['setting']->gst_presentation === 'detailed' && $item->hsn_sac)<br><span class="muted">HSN {{ $item->hsn_sac }} · GST {{ number_format((float) $item->tax_rate, 3) }}%</span>@endif</td>
