<table class="thermal-items" role="presentation">
    ...
    <tbody>
        @foreach($invoice->items as $item)
            <tr>
                <td><strong>{{ str($item->name)->limit($render['template']['paper_format'] === 'thermal_58' ? 26 : 42) }}</strong>@if($render['setting']->gst_presentation === 'detailed' && $item->hsn_sac)<br><span class="muted">HSN {{ $item->hsn_sac }} · GST {{ number_format((float) $item->tax_rate, 3) }}%</span>@endif</td>
