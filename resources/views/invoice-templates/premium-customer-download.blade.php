...
<head>
    <style>
        ...
        .signature-block { border-collapse:collapse; margin-left:auto; width:150px; } .signature-block td { padding:0; text-align:center; vertical-align:top; } .signature-heading { padding-bottom:7px !important; } .signature-image { height:43px; padding-bottom:3px !important; } .signature-image img { display:block; margin:0 auto; max-height:40px; max-width:120px; } .signature-line { border-top:1px solid #8aa6d5; line-height:1.35; padding-top:4px; text-align:center; }
...
<table class="completion" role="presentation"><tr><td class="notes" width="63%"><strong>Notes</strong><br>{{ $invoice->notes ?: ($invoice->terms_conditions ?: 'Thank you for your business. We appreciate your partnership.') }}</td><td class="signature"><table class="signature-block" role="presentation" width="150" align="right"><tr><td class="signature-heading"><strong class="accent">For {{ $invoice->company->legal_name ?: $invoice->company->name }}</strong></td></tr>@if($invoice->show_authorized_signature && data_get($render, 'signature.data_uri'))<tr><td class="signature-image"><img src="{{ $render['signature']['data_uri'] }}" alt="Authorized signature"></td></tr>@endif<tr><td class="signature-line">{{ data_get($render, 'signature.name') ?: 'Authorized Signatory' }} @if(data_get($render, 'signature.designation'))<br><span class="muted">{{ $render['signature']['designation'] }}</span>@endif</td></tr></table></td></tr></table>
