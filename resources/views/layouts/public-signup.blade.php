...
<body class="command-center-theme min-h-screen bg-slate-50 text-slate-950 antialiased dark:bg-slate-950 dark:text-slate-100">
    <main class="grid min-h-screen lg:grid-cols-[minmax(0,0.95fr)_minmax(520px,1.05fr)]">
        ...
        <section class="flex min-h-screen items-start justify-center bg-slate-50 px-4 py-8 sm:px-6 sm:py-12 lg:items-center lg:px-10 dark:bg-slate-950">
            <div class="w-full max-w-2xl"><a href="{{ route('saas.public-signup.show') }}" class="mb-8 flex items-center gap-3 lg:hidden"><span class="grid size-10 place-items-center rounded-lg bg-slate-950 text-sm font-bold text-white dark:bg-teal-300 dark:text-slate-950">RP</span><span><span class="block font-semibold text-slate-950 dark:text-white">RetailPOS</span><span class="block text-sm text-slate-500 dark:text-slate-400">Free 365</span></span></a><div class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm sm:p-8 dark:border-slate-800 dark:bg-slate-900">@yield('content')</div></div>
        </section>
    </main>
</body>
</html>
