...
<body>
    ...
    <table class="brand-table brand"><tr><td><div class="eyebrow">{{ $quotation->company?->name ?: 'RetailPOS.biz' }}</div><h1>{{ $presentation['document_title'] }}</h1><p class="muted">{{ $isGst ? 'GST quotation' : 'Non-GST quotation' }}</p></td><td class="right"><strong>{{ $quotation->quotation_number }}</strong><br><span class="muted">Issued {{ $quotation->created_at?->format('d M Y') }}</span><br><span class="muted">Valid until {{ $quotation->valid_until?->format('d M Y') ?? 'Not specified' }}</span></td></tr></table>
    ...
    <div class="section"><h2>Proposal items</h2><table class="items"><thead><tr><th>Item</th><th class="number">Qty</th><th class="number">Unit price</th><th class="number">Discount</th>@if($isGst)<th class="number">Tax</th>@endif<th class="number">Total</th></tr></thead><tbody>@foreach($quotation->items as $item)<tr><td><strong>{{ $item->name }}</strong>@if($item->description)<br><span class="muted">{{ $item->description }}</span>@endif</td><td class="number">{{ number_format((float) $item->quantity, 3) }}</td><td class="number">{{ $quotation->currency }} {{ number_format((float) $item->unit_price, 2) }}</td><td class="number">{{ $quotation->currency }} {{ number_format((float) $item->discount_amount, 2) }}</td>@if($isGst)<td class="number">{{ $quotation->currency }} {{ number_format((float) $item->tax_amount, 2) }}</td>@endif<td class="number"><strong>{{ $quotation->currency }} {{ number_format((float) $item->line_total, 2) }}</strong></td></tr>@endforeach</tbody></table></div>

    <table class="summary summary-table section"><tr><td>Subtotal</td><td class="right">{{ $quotation->currency }} {{ number_format((float) $quotation->subtotal, 2) }}</td></tr><tr><td>Discount</td><td class="right">{{ $quotation->currency }} {{ number_format((float) $quotation->discount_total, 2) }}</td></tr>@if($isGst)<tr><td>Tax</td><td class="right">{{ $quotation->currency }} {{ number_format((float) $quotation->tax_total, 2) }}</td></tr>@endif<tr class="total"><td>Grand total</td><td class="right">{{ $quotation->currency }} {{ number_format((float) $quotation->grand_total, 2) }}</td></tr></table>
